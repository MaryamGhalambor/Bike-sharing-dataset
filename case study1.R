...
# bar chart
final_trips %>% 
  ggplot(mapping = aes(x=usertype))+geom_bar(color="black",fill=c("#FF3333", "#33CCCC"))+geom_text(aes(label=..count..),stat = "count",vjust=1.5,color="black")+ labs(title = "Casual vs Members",x="USERTYPE")+theme_light()+
  theme(axis.title = element_text(size=16,lineheight = .9,family="Times",face = "bold.italic",color="red"),axis.ticks=element_blank(),axis.text.y = element_blank())

# Pie Chart
mytable <- table(final_trips$usertype)
pct<- round(100*mytable/sum(mytable), 2)
lbls <- paste(sep=" ", pct, "%") # add % to labels
pie(mytable, labels = lbls,
    main="Pie Chart of User Type\n", col = c("#FF3333", "#33CCCC"), radius = 1)
legend(1, 0.9, names(mytable), cex = 0.8, fill = c("#FF3333", "#33CCCC"))
